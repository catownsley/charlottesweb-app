...
Last assessed: 2026-03-14 | Components analyzed: 8 | Findings: 27

This threat model was generated by running Charlotte's Web against itself, using the platform's own threat modeling feature to analyze its own dependency stack. You could say the project is self-aware.

Each finding is mapped to its actual disposition based on implemented controls.
...
| # | Severity | Threat | Disposition | Control |
|---|----------|--------|-------------|---------|
| 1 | HIGH | JWT bypass via unvalidated `crit` header (CVE-2026-32597) | **Mitigated** | PyJWT upgraded to 2.12.1 (patched in 2.12.0). Algorithms explicitly configured via `algorithms` parameter in [`src/security.py`](src/security.py). |
| 2 | HIGH | TLS not enabled, credentials transmitted in plaintext (CWE-319) | **Mitigated** | `HTTPSEnforcementMiddleware` redirects all HTTP to HTTPS (301). HSTS header set with `max-age=31536000; includeSubDomains; preload`. Dev server binds exclusively on port 8443 with TLS. See [`src/middleware.py`](src/middleware.py). |
| 3 | MEDIUM | Audit logging absent (CWE-778) | **Mitigated** | Structured JSON audit logging implemented in [`src/audit.py`](src/audit.py). Covers authentication, data access, security alerts, errors, and configuration changes. See [Audit Logging](#4-audit-logging) above. |
| 4 | MEDIUM | Uvicorn log injection via ANSI escape sequences (CVE-2020-7694) | **Mitigated** | Uvicorn 0.41.0 installed. Application uses structured logging via Python `logging` module, not raw terminal output. |
| 5 | LOW | OpenAPI docs expose API schema for reconnaissance | **Mitigated** | Documentation endpoints disabled in production: `docs_url=None`, `redoc_url=None`, `openapi_url=None` when `DEBUG=false`. See [`src/main.py:149-151`](src/main.py). |
| 6 | HIGH | SQL injection via string concatenation in SQLAlchemy | **Mitigated** | All database queries use SQLAlchemy ORM with parameterized queries. No raw `text()` with user input. Pydantic validates all request inputs before they reach the ORM layer. |

#### Not Applicable

| # | Severity | Finding | Disposition | Rationale |
|---|----------|---------|-------------|-----------|
| 1 | HIGH | HIPAA compliance gaps (MFA, encryption at rest, audit controls) | **Not Applicable** | This application processes software component metadata and vulnerability data. It does not store, transmit, or process Protected Health Information (PHI). HIPAA requirements do not apply to this workload. |
| 2 | HIGH | FastAPI CVE-2024-40627 (authorization bypass) | **Not Applicable** | Affects `fastapi-opa` middleware package, not core FastAPI. Package is not installed in this application. |
| 3 | HIGH | FastAPI CVE-2024-42816, CVE-2024-42818 (admin panel vulnerabilities) | **Not Applicable** | Affects `fastapi-admin-pro` v0.1.4, a third-party admin UI package. Package is not installed in this application. |
| 4 | HIGH | Python CVE-2020-29396 (sandbox escape) | **Not Applicable** | Affects Odoo's Python sandboxing mechanism. Odoo is not installed or used in this application. |
| 5 | MEDIUM | Python CVE-2021-32052 (URLValidator header injection) | **Not Applicable** | Affects Django's `URLValidator`. Django is not installed or used in this application. |
| 6 | MEDIUM | Uvicorn CVE-2025-27519 (path traversal) | **Not Applicable** | Affects Cognita's local upload endpoint, not core Uvicorn. Cognita is not installed in this application. |
| 7 | MEDIUM | Pydantic CVE-2025-22151 (relay ID injection) | **Not Applicable** | Affects `strawberry-graphql` relay integration. Strawberry GraphQL is not installed in this application. |

#### Accepted Risk

| # | Severity | Finding | Disposition | Rationale |
|---|----------|---------|-------------|-----------|
| 1 | HIGH | SQLite database file unencrypted at rest | **Accepted (Dev)** | SQLite is used for development only. Production deployment guidance recommends PostgreSQL with storage-level encryption. See [Production Security Checklist](#before-deployment). Database file permissions restricted to application service account. |
| 2 | MEDIUM | SQLite file-level locking under concurrent writes | **Accepted (Dev)** | SQLite concurrency limitations are acceptable for development and single-user workflows. WAL mode is recommended. Production guidance specifies PostgreSQL migration. |
| 3 | MEDIUM | SSRF risk via python-requests outbound calls | **Accepted** | Outbound HTTP calls are made only to known external services (NVD API) with hardcoded base URLs. No user-supplied URLs are passed to `requests`. URL allowlisting is recommended if user-controlled URLs are added in the future. |

#### Remediation Planned

| # | Severity | Finding | Plan |
|---|----------|---------|------|
| 1 | HIGH | No Multi-Factor Authentication | MFA is recommended for production deployments with user-facing authentication. Current authentication uses API keys and JWT tokens. TOTP-based MFA (e.g., `pyotp`) is planned for a future release when user login flows are added. |
| 2 | MEDIUM | Starlette unbounded multipart form field buffering (CVE-2024-47874) | Verify bundled Starlette version in FastAPI 0.135.1. Upgrade to Starlette 0.40.0+ if necessary. Add reverse proxy request body size limits for production deployments. |

### Dependency Vulnerability Summary

| Component | Version | CVEs Found | Status |
|-----------|---------|------------|--------|
| SQLite | 3.50.4 | CVE-2022-31631 | Mitigated: CVE affects PHP `PDO::quote()`, not Python `sqlite3`. SQLAlchemy uses parameterized queries. |
| PyJWT | 2.12.1 | CVE-2026-32597 | Patched: Fixed in 2.12.0, running 2.12.1. |
| Python | 3.14.3 | CVE-2020-29396, CVE-2021-32052 | Not Applicable: Affect Odoo and Django respectively, neither installed. |
| FastAPI | 0.135.1 | CVE-2024-40627, CVE-2024-42816, CVE-2024-42818, CVE-2024-47874 | 3 Not Applicable (third-party packages). CVE-2024-47874 (Starlette): under review. |
| Uvicorn | 0.41.0 | CVE-2020-7694, CVE-2025-27519 | CVE-2020-7694: Mitigated (structured logging). CVE-2025-27519: Not Applicable (Cognita). |
| Pydantic | 2.12.5 | CVE-2025-22151 | Not Applicable: Affects Strawberry GraphQL, not installed. |

### Compound Risk Assessment

| Severity | Risk | Status |
|----------|------|--------|
| CRITICAL | JWT bypass (CVE-2026-32597) + absent MFA + absent TLS | **Reduced to LOW**: PyJWT patched (2.12.1), TLS enforced via middleware and HSTS. MFA planned for future release. |
| CRITICAL | SQLite SQL injection (CVE-2022-31631) + raw queries + unencrypted DB | **Reduced to LOW**: CVE is PHP-specific. All queries use ORM parameterization. SQLite encryption at rest is accepted risk for dev environment. |
| MEDIUM | Starlette multipart DoS (CVE-2024-47874) + single-server deployment | **Under Review**: Verifying bundled Starlette version. Production guidance recommends nginx reverse proxy with body size limits. |
...
- **Status: Not Applicable.** This application processes software component metadata and vulnerability data. It does not store, transmit, or process Protected Health Information (PHI). HIPAA requirements do not apply to this workload.
- This determination is documented in the [Threat Model Assessment](#threat-model-assessment) above.
- If the application scope changes to include PHI in the future, a full HIPAA gap analysis must be conducted before deployment.
...
**Security Posture:** Threat model assessed, 6 mitigated, 7 not applicable, 3 accepted risk, 2 remediation planned
